export default function RecordingControls({ recordingState, elapsedSeconds, onStart, onStop }) {
  ...
  if (recordingState === 'recording') {
    return (
      <div className="flex flex-col items-center gap-3">
        ...
        <p className="text-sm font-semibold text-[#00B5C8]">{formatTime(elapsedSeconds)}</p>
  ...
  return (
    <div className="flex flex-col items-center gap-4">
      ...
      <button
        onClick={onStart}
        className="w-20 h-20 rounded-full bg-white border-2 border-[#00B5C8] flex items-center justify-center shadow-sm active:scale-95 transition-transform hover:bg-cyan-50"
      >
