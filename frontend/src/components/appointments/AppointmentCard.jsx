    ...
    <div className={`bg-white rounded-2xl border shadow-sm p-4 ${isCancelled ? 'border-gray-100 opacity-50' : isPast ? 'border-gray-100 opacity-75' : 'border-gray-100'}`}>
      ...
      {!isCancelled && (
        <button
          ...
          className="mt-3 w-full py-2 px-4 bg-cyan-50 border border-cyan-200 text-[#00B5C8] text-sm font-medium rounded-xl flex items-center justify-center gap-2 active:bg-cyan-100"
        >
      ...
      {appointment.google_meet_link && !isPast && (
        <a
          ...
          target="_blank"
          rel="noreferrer"
          className="mt-3 flex items-center justify-center gap-2 w-full py-2 rounded-xl border border-[#00B5C8] text-[#00B5C8] text-sm font-medium active:opacity-80"
        >
